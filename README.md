# Portfolio: Aniket Yadav

Welcome to my professional portfolio! This repository contains my personal projects and showcases my work in Full-Stack Web Development and AI/ML technologies.

[![Aniket's Portfolio](https://img.shields.io/badge/Portfolio-Aniket_Yadav-000000?style=plastic&logo=vercel&logoColor=white)](https://portpholio-sandy.vercel.app/)

## 🛠 Tech Stack
*   **Web Development**: HTML, CSS, JavaScript
*   **AI/ML**: Python, Machine Learning, NLP
*   **Tools**: Git, GitHub, VS Code, Netlify

## 📂 Projects
*   **AI-Driven Microfinance Loan Risk Prediction**: A hybrid assessment system using XGBoost.
*   **Fake News Detection System**: An NLP-based misinformation classification project.
*   **AURA AI Career Co-Pilot**: A career-focused agent for profile matching.
*   **Voice-Based Virtual Assistant**: A modular Python-based automation tool.

## 🎓 Academic Background
*   **University**: Babu Banarasi Das University (BBDU)
*   **Status**: Third-year B.Tech Engineering Student

---
*Created by Aniket Yadav*
